...
public class JDBCExample1 {
	...
	try {
		...
		/*2-1) Oracle JDBC Driver 객체를 메모리카드에 로드(적재)하기*/
		...
		// - Oracle DBMS 연결시 필요한 코드가 담겨있는 클래스
		//	 (Oracle에서 만들어서 준 클래스)
		...
		/* 3. SQL 작성 */
		// 주의사항
		// -> JDBC 코드에서 SQL 작성시 세미콜론(;)을 작성하면 안된다.
		...
		/* 5. Statement 객체를 이용해서 SQL수행 후 결과 반환 받기 */
		// 1) ResultSet Statement.encuteQuery(sql);
		// -> SELECT문 실행 매서드.
		//	  결과로 java.sql.ResultSet 반환
		
		// 2) int Statement.excuteUpdate(sql);
		// -> DML(INSERT, UPDATE, DELETE) 실행 메서드
		// 	  결과로 int반환(삽입, 수정, 삭제된 행의 개수)
